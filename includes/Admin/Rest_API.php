...
class Rest_API {
	 ...
	 * @var string
	 */
	private const NAMESPACE = 'acf-repeater/v1';
	...
	public function get_repeater_field( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $field_key || ! $post_id ) {
			return new \WP_Error( 'missing_params', __( 'Field key and post ID are required.', 'acf-repeater' ), array( 'status' => 400 ) );
		}

		$field = acf_get_field( $field_key );
		if ( ! $field || $field['type'] !== 'repeater' ) {
			return new \WP_Error( 'not_found', __( 'Repeater field not found.', 'acf-repeater' ), array( 'status' => 404 ) );
		}
	...
	public function get_repeater_row( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! is_array( $value ) || ! isset( $value[ $row_index ] ) ) {
			return new \WP_Error( 'not_found', __( 'Row not found.', 'acf-repeater' ), array( 'status' => 404 ) );
		}
	...
	public function add_repeater_row( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $field_key || ! $post_id ) {
			return new \WP_Error( 'missing_params', __( 'Field key, post ID, and row data are required.', 'acf-repeater' ), array( 'status' => 400 ) );
		}
		...
		return new \WP_Error( 'save_failed', __( 'Failed to save row.', 'acf-repeater' ), array( 'status' => 500 ) );
	}
	...
	public function update_repeater_row( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $field_key || ! $post_id ) {
			return new \WP_Error( 'missing_params', __( 'Field key, post ID, and row index are required.', 'acf-repeater' ), array( 'status' => 400 ) );
		}
		...
		return new \WP_Error( 'save_failed', __( 'Failed to update row.', 'acf-repeater' ), array( 'status' => 500 ) );
	}
	...
	public function delete_repeater_row( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		return new \WP_Error( 'delete_failed', __( 'Failed to delete row.', 'acf-repeater' ), array( 'status' => 500 ) );
	}
	...
	public function get_repeater_schema(): array {
		return array(
			'description' => __( 'Repeater field data.', 'acf-repeater' ),
			'type'        => 'object',
			'properties'  => array(
				'field_key'      => array(
					'type'        => 'string',
					'description' => __( 'ACF field key.', 'acf-repeater' ),
				),
				'field_name'     => array(
					'type'        => 'string',
					'description' => __( 'ACF field name.', 'acf-repeater' ),
				),
				'layout'         => array(
					'type'        => 'string',
					'enum'        => array( 'table', 'block' ),
					'description' => __( 'Layout type.', 'acf-repeater' ),
				),
				'button_label'   => array(
					'type'        => 'string',
					'description' => __( 'Add row button label.', 'acf-repeater' ),
				),
				'min_rows'       => array(
					'type'        => 'integer',
					'description' => __( 'Minimum rows.', 'acf-repeater' ),
				),
				'max_rows'       => array(
					'type'        => 'integer',
					'description' => __( 'Maximum rows.', 'acf-repeater' ),
				),
				'collapsed'      => array(
					'type'        => 'string',
					'description' => __( 'Collapsed field key.', 'acf-repeater' ),
				),
				'sortable'       => array(
					'type'        => 'boolean',
					'description' => __( 'Rows sortable.', 'acf-repeater' ),
				),
				'duplicate'      => array(
					'type'        => 'boolean',
					'description' => __( 'Rows duplicatable.', 'acf-repeater' ),
				),
				'delete_confirm' => array(
					'type'        => 'boolean',
					'description' => __( 'Delete confirmation.', 'acf-repeater' ),
				),
				'rows'           => array(
					'type'  => 'array',
					'items' => array(
